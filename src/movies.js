...
function orderAlphabetically(movies) {
  let sortedMov = movies.map((name) => {
    return name.title;
  });

  let moreThan20 = [];
  if (sortedMov.length > 20) {
    for (let i = 0; i < 20; i++) {
      moreThan20.push(sortedMov[i]);
    }
    return moreThan20.sort();
  }
  return sortedMov.sort();
}
